    ...
    const verifyPaymentAndSendOrder = async () => {
      ...
      try {
        ...
        const verifyResponse = await axios.post(
          "http://localhost:3000/api/payment/verify-session",
          { sessionId }
        );
  ...
  const sendOrder = async () => {
    try {
      ...
      const orderResponse = await axios.post(
        "http://localhost:3000/order/process-order",
        formData,
        {
          headers: {
...
//       // Build order items array (include id so you can fetch the file from context)
//       const orderItems = cartItems.map((item) => ({
...
//       // Append each PDF file from the cart items using the context to get the actual File
//       for (let index = 0; index < cartItems.length; index++) {
//         const item = cartItems[index];
//         // Retrieve the actual file object using the id stored in Redux
//         const file = getCartFile(item.id);
//         if (file) {
//           const fileName = file.name || `order_${Date.now()}_${index}.pdf`;
...
//       console.log("Order processed successfully:", orderResponse.data);
//       // Clear the cart in Redux and context
//       dispatch(clearCart());
//       clearCartFiles();
//     } catch (err) {
//       console.error("Error sending order:", err);
